...
| **source_tsx** | `source_tsx` | string | if `react` | ≤512 KiB | Author source for `react` pages: JSX + light TS (`type/interface`, `: T`, `as T`, `<T>`) + `import ... from 'react'` allowed; other packages banned. Click **Build** to validate into `bundle_js`. | `instance_page_handler.go:130` `validateReactSource` |
