...
| **content_type** | `content_type` | string | **yes** | `html` \| `markdown` \| `blocks` \| `react` | Empty on create → defaults to `markdown`. `react` = near-real: JSX + light TS + react-only imports, transpiled at render. | `instance_page_handler.go:83` `validContentTypes` |
...
| **source_tsx** | `source_tsx` | string | if `react` | ≤512 KiB | Author source for `react` pages: JSX + light TS (`type/interface/enum/namespace`, `: T`, `as T`, `<T>`) + `import ... from 'react'` and `export` on plain declarations allowed; other packages/modules banned. Click **Build** to validate into `bundle_js`. | `instance_page_handler.go:130` `validateReactSource` |
