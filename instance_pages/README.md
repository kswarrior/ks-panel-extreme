...
- `source_tsx` (required if content_type=react): author React source — JSX + light TS (`type/interface/enum/namespace`, `: T`, `as T`, `<T>`) + `import ... from 'react'` and `export` on plain declarations allowed, other packages/modules banned (`function Page() { … }` + `return Page;`, `React.createElement` still works); click **Build** in the Studio to validate it into the executable bundle (`bundle_js`)
