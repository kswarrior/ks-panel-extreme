...
// ConsoleHandler is the chi handler bound at
// /api/instances/{id}/console. It bridges the browser to the instance's
// MAIN process stdio (the template's startup command, e.g. a Minecraft
// server) through ksedge /api/edge/attach — the startup-console twin of
...
func ConsoleHandler(w http.ResponseWriter, r *http.Request) {
	...
	if mode == "reverse_tunnel" {
		if !tunnel.Global().IsConnected(node.ID) {
			http.Error(w, "edge not connected via WSS tunnel (reverse_tunnel console requires edge to be online)", http.StatusBadGateway)
			return
		}
		http.Error(w, "console over WSS tunnel not yet implemented for reverse_tunnel; use direct or local_port mode for console access", http.StatusNotImplemented)
		return
	}
	if mode == "both" || mode == "local_both" {
		if !tunnel.Global().IsConnected(node.ID) {
			...
			if route.Strict && route.Transport == edge.TransportWSS {
				http.Error(w, "edge not connected via WSS tunnel (console task prefers WSS with fallback disabled)", http.StatusBadGateway)
				return
			}
		}
	}
	...
	// instead of leaking one goroutine per console tab. The deferred
