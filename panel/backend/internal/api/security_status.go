...
// around SecurityMiddleware) with SPA-safe CSP and WS/static/Bearer
// bypasses; the SPA mints tokens via GET /api/csrf-token.
//
// NOTE: the live route is handlers.SecurityStatusHandler (mounted at
// /api/security/status in server.go); this copy is kept in sync so both
// report enforced:true when wired.
func SecurityStatusHandler(w http.ResponseWriter, r *http.Request) {
	...
	_ = json.NewEncoder(w).Encode(map[string]interface{}{
		...
		"csrf": map[string]interface{}{
			// Token middleware IS mounted globally (CSRFMiddleware).
		...
		"security_headers": map[string]interface{}{
			...
			"note": "SecurityHeadersMiddleware + XSS/Sanitize/Validation mounted " +
				"globally in NewRouter (after cors, around SecurityMiddleware).",
		},
