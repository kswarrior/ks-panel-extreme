...
CREATE INDEX IF NOT EXISTS idx_application_permissions_app   ON application_permissions(application_id);
CREATE INDEX IF NOT EXISTS idx_application_installations_app ON application_installations(application_id);
...
-- (PostgreSQL has no INSERT OR IGNORE; ON CONFLICT DO NOTHING is the
-- equivalent idempotent form. The sqlite/mysql dirs keep their own verbs.)
