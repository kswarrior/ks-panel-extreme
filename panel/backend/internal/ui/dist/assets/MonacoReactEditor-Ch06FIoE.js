import{r as f,j as S}from"./react-0h_Dc6Wy.js";import{U as A,e as y,W as R,a as E,l as m}from"./monaco-BGNI_53U.js";const C=`// Custom Page SDK — provides a runtime API for custom instance pages.
...
export type { CustomPageAPI as KSPageSDK };`,I=["InstanceContext","ActionType","PageAction","ActionResult","PageActionDef","FileEntry","ChartSeriesPoint","ChartSeries","ChartOptions","PageWSEndpoint","PageWSOptions","CustomPageAPI"];function O(e,r){const i=new RegExp(`export\\s+(interface\\s+${r}\\b|type\\s+${r}\\b)`).exec(e);if(!i||i.index===void 0)return null;const s=$(e);if(i[1].startsWith("interface")){const n=s.indexOf("{",i.index);if(n<0)return null;let t=0;for(let d=n;d<s.length;d++)if(s[d]==="{")t++;else if(s[d]==="}"&&(t--,t===0))return v(e.slice(i.index,d+1));return null}const a=s.indexOf("=",i.index);if(a<0)return null;let o=0;for(let n=a+1;n<s.length;n++){const t=s[n];if(t==="{"||t==="("||t==="[")o++;else if(t==="}"||t===")"||t==="]")o--;else if(t===";"&&o===0)return v(e.slice(i.index,n+1))}return null}function $(e){const r=e.split(""),i=(a,o)=>{for(let n=a;n<o;n++)r[n]!==`
`&&(r[n]=" ")};let s=0;const l=e.length;for(;s<l;){const a=e[s],o=e[s+1];if(a==="/"&&o==="/"){let n=s;for(;n<l&&e[n]!==`
`;)n++;i(s,n),s=n}else if(a==="/"&&o==="*"){const n=e.indexOf("*/",s+2),t=n<0?l:n+2;i(s,t),s=t}else if(a==="'"||a==='"'||a==="`"){const n=a;let t=s+1;const d=[];for(;t<l;){if(e[t]==="\\"){t+=2;continue}if(n==="`"&&e[t]==="$"&&e[t+1]==="{"){let c=t+2,u=1;for(;c<l&&u>0;)e[c]==="{"?u++:e[c]==="}"&&u--,c++;d.push([t,c]),t=c;continue}if(e[t]===n||n!=="`"&&e[t]===`
`)break;t++}const h=Math.min(t+1,l);i(s,h);for(const[c,u]of d)for(let p=c;p<Math.min(u,l);p++)r[p]=e[p];s=h}else s++}return r.join("")}function v(e){return e.replace(/^export\s+/,"")}const w=`
...
`;let g=null;function M(){if(g!==null)return g;try{const e=[];for(const r of I){const i=O(String(C??""),r);i&&e.push(i)}if(!e.some(r=>/(^|\n)interface\s+CustomPageAPI\b/.test(r)))throw new Error("CustomPageAPI block not found in customPageSdk.ts");g=`${e.join(`

`)}
${w}`}catch{g=`declare const sdk: any;
${w}`}return g}let k=!1;function N(){k||(k=!0,self.MonacoEnvironment={getWorker(e,r){return r==="typescript"||r==="javascript"?new R:new E}})}let P=!1;function _(){P||(P=!0,m.typescript.typescriptDefaults.setCompilerOptions({target:m.typescript.ScriptTarget.ES2020,allowNonTsExtensions:!0,moduleResolution:m.typescript.ModuleResolutionKind.NodeJs,jsx:m.typescript.JsxEmit.React,allowJs:!0,checkJs:!1,strict:!1,noEmit:!0}),m.typescript.typescriptDefaults.setDiagnosticsOptions({noSemanticValidation:!1,noSyntaxValidation:!1}),m.typescript.typescriptDefaults.addExtraLib(M(),"file:///ks-studio/sdk.d.ts"))}let T=!1;function H(){T||(T=!0,y.defineTheme("ks-dark",{base:"vs-dark",inherit:!0,rules:[],colors:{"editor.background":"#0a0a0f","editor.lineHighlightBackground":"#ffffff0d"}}))}const U=({value:e,onChange:r,fileKey:i,ariaLabel:s})=>{const l=f.useRef(null),a=f.useRef(null),o=f.useRef(null),n=f.useRef(!1),t=f.useRef(r);t.current=r,f.useEffect(()=>{N(),_(),H();const h=l.current;if(!h)return;const c=A.parse(`file:///ks-studio/${i==="index"?"index":i}.tsx`),u=y.getModel(c),p=u??y.createModel(e,"typescript",c);if(u&&u.getValue()!==e){n.current=!0;try{u.setValue(e)}finally{n.current=!1}}o.current=p;const b=y.create(h,{model:p,language:"typescript",theme:"ks-dark",automaticLayout:!0,fontSize:13,fontFamily:"ui-monospace, SFMono-Regular, Menlo, Consolas, monospace",minimap:{enabled:!1},scrollBeyondLastLine:!1,padding:{top:8},lineNumbers:"on",renderLineHighlight:"all",tabSize:2,insertSpaces:!0,wordWrap:"on",stickyScroll:{enabled:!1},fixedOverflowWidgets:!0,ariaLabel:s});a.current=b;const x=p.onDidChangeContent(()=>{n.current||t.current(p.getValue())});return()=>{x.dispose(),b.dispose();try{p.dispose()}catch{}a.current=null,o.current=null}},[]);const d=f.useRef(e);return f.useEffect(()=>{d.current=e;const h=o.current;if(!(!h||h.getValue()===e)){n.current=!0;try{h.setValue(e)}finally{n.current=!1}}},[e]),S.jsx("div",{ref:l,style:{height:"420px",width:"100%"},className:"rounded-md overflow-hidden border border-white/10"})};export{U as default};
