              ...
              <article key={u.id} id={`ks-user-${u.id}`} className={`ks-card ks-list-card glass-card rounded-xl overflow-hidden flex flex-col hover:border-white/20 transition-colors ${isSuspended ? 'border-red-500/30' : ''}`}>
                ...
                <div className="px-4 pt-7 flex flex-col gap-3 flex-1 min-w-0">
                  ...
                  <footer className="mt-auto pt-2 border-t border-white/[0.06] flex items-center justify-between gap-2">
                    ...
                    <CardMenu
                      ariaLabel={`Actions for user ${u.username}`}
                      items={[
                        {
                          kind: 'checkbox',
                          key: 'notify-login',
                          label: 'Notify on login',
                          checked: !!notifyLogin[u.id],
                          hint: notifyLogin[u.id]
                            ? 'Email me when this user logs in'
                        ...
                        },
                        {
                          ...
                          key: 'require-mfa',
                          label: 'Require MFA',
                          checked: !!requireMfa[u.id],
                          hint: requireMfa[u.id]
                            ? 'Two-factor enforced for this account'
                            : 'Optional 2FA',
                        },
                        { kind: 'separator', key: 'sep1' },
                        {
                          kind: 'submenu',
                          key: 'role',
                          label: roles.find((r) => r.id === u.role_id)?.name || 'Set role…',
                          children:
                            roles.length === 0
                              ? [
                                  {
                        ...
                        { kind: 'separator', key: 'sep2' },
                        {
                          kind: 'submenu',
                          key: 'suspend',
                          label: isSuspended ? 'Unsuspend user' : 'Suspend user…',
                          icon: (
                      ...
                      onSelect={(key) => {
                        if (key === 'view') navigate(`/user/${u.id}`);
                        else if (key === 'edit') navigate(`/users/${u.id}/edit`);
                        else if (key === 'delete') remove(u);
                        else if (key === 'unsuspend') unsuspend(u);
                        else if (key === 'suspend-indefinite') suspend(u);
                        else if (key === 'suspend-1h') suspend(u, 1);
                        else if (key === 'suspend-24h') suspend(u, 24);
                        else if (key === 'suspend-7d') suspend(u, 24 * 7);
                        else if (key === 'suspend-30d') suspend(u, 24 * 30);
                      }}
                      onToggle={(key, next) => {
                        if (key === 'notify-login') {
                          setNotifyLogin((m) => ({ ...m, [u.id]: next }));
                        } else if (key === 'require-mfa') {
                          setRequireMfa((m) => ({ ...m, [u.id]: next }));
                        } else if (key.startsWith('role-')) {
                          const rid = Number(key.slice(5));
                          if (next) {
                            setUsers((us) =>
                              us.map((x) =>
                                x.id === u.id ? { ...x, role_id: rid } : x,
                              ),
                            );
                          }
                        }
