  ...
  <>
    ...
    <div className={sectionCls}>
      <div className="flex items-center justify-between mb-1">
        ...
        <button type="button" onClick={onActionAdd} className={addBtn}>+ Action</button>
      </div>
      ...
      <div className="space-y-4">
        {actions.map((a, i) => (
          <div key={i} className="border border-white/10 rounded-md p-3 space-y-3 bg-black/30">
            {/* Action header — id is the trigger handle */}
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
              <div>
                ...
                <input value={a.id} onChange={(e) => {
                  ...
                  onActionUpdate(i, { id: v });
                }} placeholder="start_server" className={monoCls + ' border-emerald-700/40 focus:border-emerald-400'} />
              </div>
              <div>
            ...
            <div className="flex flex-wrap gap-4">
              ...
              <button type="button" onClick={() => onActionDelete(i)} className="ml-auto text-xs text-red-400 hover:text-red-200">Remove action</button>
            </div>

            {/* Session semantics — auto-start/stop + command filter */}
            ...
            <div className="pt-1">
              ...
              <p className="text-[11px] text-gray-500 mt-1">
                <code>same</code>: sends the command to the process's console (e.g. Minecraft reads "stop" from stdin).
                <code>different</code>: runs a separate exec (e.g. pkill -f java).
              </p>
            </div>
            ...
            {a.session === 'long_running' && (
              <p className="text-[11px] text-gray-500">long_running: edge boots the instance when this action fires, runs the steps once, then keeps the foreground process alive. When the main process exits, the instance is stopped automatically (or restarted if "Restart on crash" is on).</p>
            )}
            {a.session === 'vm_full' && (
              <p className="text-[11px] text-gray-500">vm_full: edge gives you a raw LXD/KVM/multipass console. Everything the user types is forwarded as-is. Use only when you trust the user (typically admin-only).</p>
            )}
            ...
            <div className="pl-3 border-l-2 border-emerald-700/40 space-y-3 mt-2">
              <div className="flex items-center justify-between">
                ...
                <button type="button" onClick={() => onActionStepAdd(i)} className="text-xs text-emerald-300 hover:text-emerald-100 underline">+ Step</button>
              </div>
              {a.steps.map((s, j) => (
                <div key={j} className="border border-white/10 rounded-md p-3 space-y-2 bg-black/20">
                  <div className="flex gap-2 items-center">
                    ...
                    <button type="button" onClick={() => onActionStepDelete(i, j)} className="ml-auto text-xs text-red-400 hover:text-red-200">Remove</button>
                  </div>
                  {s.action === 'shell' && (
                    <input value={s.command} onChange={(e) => onActionStepUpdate(i, j, { command: e.target.value })} placeholder="/entrypoint.sh --config {{CONFIG_PATH}}" className={monoCls} />
                  )}
                  {s.action === 'download' && (
                  ...
                  {s.action === 'mkdir' && (
                    <input value={s.path} onChange={(e) => onActionStepUpdate(i, j, { path: e.target.value })} placeholder="directory path (with -p)" className={monoCls} />
                  )}
                  {s.action === 'git_clone' && (
                    <div className="grid grid-cols-2 gap-2">
                  ...
                  {s.action === 'pip_install' && (
                    <input value={s.command} onChange={(e) => onActionStepUpdate(i, j, { command: e.target.value })} placeholder="requirements / package" className={monoCls} />
                  )}
                  {s.action === 'npm_install' && (
                    <input value={s.command} onChange={(e) => onActionStepUpdate(i, j, { command: e.target.value })} placeholder="package or empty" className={monoCls} />
                  )}
                  {s.action === 'http_check' && (
                    <input value={s.url} onChange={(e) => onActionStepUpdate(i, j, { url: e.target.value })} placeholder="URL to check" className={monoCls} />
                  )}
                  <div className="flex flex-wrap items-center gap-3">
                    <label className="text-xs text-gray-400">Retries:
                      <input type="number" min="0" value={s.retries} onChange={(e) => onActionStepUpdate(i, j, { retries: e.target.value })} className={glassFieldClass + ' w-16 ml-2'} />
                    </label>
                    <label className="inline-flex items-center gap-2 cursor-pointer">
            ...
            <p className="text-[11px] text-gray-500 font-mono">
              Triggered via <span className="text-emerald-300">POST /api/instances/&#123;id&#125;/actions/{a.id || '<id>'}/invoke</span>
            </p>
          </div>
        ))}
      </div>
