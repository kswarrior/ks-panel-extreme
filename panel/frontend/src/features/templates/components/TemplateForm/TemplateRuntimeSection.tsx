        ...
        {kind === 'docker' && (
          <>
            ...
            <div className="mt-3 rounded-md border border-sky-700/30 bg-sky-950/20 p-2 space-y-2">
              <div>
                <label className="block text-[11px] text-gray-400 mb-0.5">Terminal ID (optional — binds the startup terminal to terminal panes)</label>
                <input value={advanced.startup_terminal_id} onChange={(e) => { const v = e.target.value.toLowerCase().replace(/\s+/g, '_').replace(/[^a-z0-9_-]/g, ''); onAdvancedUpdate({ startup_terminal_id: v }); }} placeholder="e.g. main-terminal (empty = no dedicated startup terminal)" className={monoCls + ' border-sky-700/40 focus:border-sky-400'} />
