...
const TemplateForm: React.FC = () => {
  ...
  return (
    <>
      {/* Bottom-right form actions — undo / redo + Save/Create;
      ...
      <div className="grid grid-cols-1 lg:grid-cols-[220px_minmax(0,1fr)] gap-4">
        ...
        {tab === 'actions' && (
          ...
          <TemplateActionsSection
            ...
            onActionAdd={() => setForm((f) => ({ ...f, actions: [...f.actions, { id: '', name: '', description: '', icon_svg: '', icon_color: '', allowed_states: '', requires_online: false, async_run: false, run_on_create: false, cooldown_s: '0', user_invokable: false, session: 'long_running', auto_start_instance: false, auto_stop_on_exit: false, restart_on_failure: false, allowed_commands: '', blocked_commands: '', max_runtime_s: '0', stop_command: '', stop_mode: 'different', terminal_id: '', terminal_stop_on_exit: true, terminal_allow_input: 'all', terminal_allowed_commands: '', terminal_blocked_commands: '', terminal_timeout_s: '', steps: [] }] }))}
        ...
        {tab === 'pages' && (() => {
          // Full-page Configure drill-in: clicking Configure on a top-level
          // page (never a sub-page) replaces the whole Pages tab with the
          // Configure form — same content column as General / Install /
          // Actions, a real full page with back navigation.
          const cfgPage = configureIdx !== null ? form.pages[configureIdx] ?? null : null;
          if (cfgPage && configureIdx !== null) {
            return (
              <div className="space-y-4">
                <TemplatePageConfigureView
                  page={cfgPage}
                  ...
                  onConfigChange={(next) => setForm((f) => { const p2 = [...f.pages]; p2[configureIdx] = { ...p2[configureIdx], config: next }; return { ...f, pages: p2 }; })}
                />
          ...
          <div className="space-y-4">
            <div className="ks-card ks-form-card rounded-md space-y-2">
              ...
              <p className="text-[11px] text-gray-500">
                {form.home_page.trim() !== '' ? (
                  <>Opens <code className="font-mono text-sky-300">/{form.home_page.trim().replace(/^\/+|\/+$/g, '') || '…'}</code> on card click. Use a page Path from the list below{form.pages.filter((p) => p.enabled !== false && p.slug && !String(p.slug).includes('/')).map((p) => String(p.slug).trim()).filter(Boolean).length > 0 ? <> ({form.pages.filter((p) => p.enabled !== false && p.slug && !String(p.slug).includes('/')).map((p) => String(p.slug).trim()).filter(Boolean).join(', ')})</> : null} or a built-in (overview, ports, sftp, files, terminal). Unknown slugs fall back to Home.</>
                ) : (
                  ...
                  return (
                    <div key={p.slug + ':' + i} className={`border border-white/10 rounded-md bg-black/30 overflow-hidden ${p.enabled ? '' : 'opacity-60'}`}>
                      <div className="p-3 flex items-center gap-3 flex-wrap">
                        ...
                        {((p.configure?.length ?? 0) > 0 || Object.keys(p.config ?? {}).length > 0) && (
                          <button
                            type="button"
                            ...
                            title="Configure page variables — opens as a full page"
                          >
                            Configure
                          </button>
                        )}
