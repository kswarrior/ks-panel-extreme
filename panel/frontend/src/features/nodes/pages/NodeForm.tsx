...
const NodeForm: React.FC = () => {
  ...
  return (
    <>
      {/* Bottom-right form actions — fixed, corner-anchored (right-4,
          w-auto) so the phone Tabs pill shares the same bottom line on the
          left instead of floating above with an empty gap underneath
      ...
      {/* Phone tabs sit bottom-LEFT on the same bottom line as the Save/
          ...
          (instance-form pattern). */}
      <PageTabsPill ariaLabel="Node form sections" spacer={false} activeLabel={NODEFORM_TABS.find((t) => t.id === tab)?.label} outerClassName="lg:hidden fixed left-4 bottom-[max(0.75rem,env(safe-area-inset-bottom))] z-30 flex justify-start w-auto max-w-[calc(100vw-2rem)] ks-tabs-pill-wrap-below">
        {NODEFORM_TABS.map((t) => (
          <button
            key={t.id}
            type="button"
