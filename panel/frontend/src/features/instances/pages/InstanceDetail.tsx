...
//
// Terminal, Files, Ports, Env and Automation are custom instance pages
// (instance_pages/pages/*.json).
...
const NoPagesState: React.FC<{ slug: string }> = ({ slug }) => (
  <div className="glass-card rounded-xl text-center text-gray-400 space-y-2">
    ...
    <p className="text-xs text-gray-500 pb-3">
      Import pages (Home, Terminal, Files, Ports, Env, Metrics, …) from the Instance Pages library in the template or deploy editor. Automation always works — it lives in the floating instance menu.
    </p>
...
export const InstanceDynamicPage: React.FC = () => {
  ...
  // Terminal, Files, Ports, Env and Automation are custom instance pages
  // now (instance_pages/pages/*.json) — they resolve through the
  // spec.pages whitelist below.

  // SFTP card is a built-in page (not a custom spec.pages entry).
  ...
  // when the spec has no "sftp" row (the SFTP view calls the same GET
  // via fetchPanel('/sftp')).
  if (effectiveSlug === 'sftp') {
    return <InstanceSftpCard instanceId={instanceId} />;
  }

  // Overview is a native built-in like SFTP (not a custom
