...
interface InstanceAdvancedTabContentProps {
  selectedTemplate: { image: string; kind: DriverKind } | null;
  specPreview: string;
}

// InstanceAdvancedTabContent — the chromeless body behind every advanced
// deploy tab (Environment / Env Variables / Actions / Install / Runtime /
// Labels & Devices / Healthcheck / Pages / Instance Controls / Spec
// Preview). No page chrome, no tab nav — the parent (InstanceForm's
// single-page tab layout, or InstanceAdvancedOptionsFullScreen for the
// edit flow) renders the tabs and wraps this. Editor state comes from
// useDeployForm so edits propagate instantly wherever it is mounted.
const InstanceAdvancedTabContent: React.FC<InstanceAdvancedTabContentProps> = ({
  selectedTemplate,
  specPreview,
}) => {
  ...
  const { editor, setEditor, tab } = useDeployForm();
...
export default InstanceAdvancedTabContent;
